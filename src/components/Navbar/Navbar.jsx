            ...
            <div className="flex-1 hidden lg:flex items-center justify-center space-x-4 ml-16">
              {navItems.map((item) => (
                <Link
                  key={item.label}
                  to={item.href}
                  className="text-xs text-codGray-900 dark:text-codGray-50 hover:text-codGray-700 dark:hover:text-codGray-300 px-3 py-2 font-medium"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="flex-1 hidden lg:flex items-center justify-end space-x-2 tablet:space-x-4 laptop:space-x-6">
              {user ? (
                <Menu as="div" className="relative">
                  ...
                  >
                    <Menu.Items className="absolute right-0 mt-2 w-48 bg-white dark:bg-background-dark rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                      <Menu.Item>
                        {({ active }) => (
                          <Link
                            to="/dashboard"
